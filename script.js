...
let veryOldInventors = inventors.filter((e) => {
...
// let inventorNames = inventors.map((e) => {
//     return {
//         first : e.first,
//         last  : e.last
...
let inventorNames = inventors.map(({first, last}) => ({first, last}));
...
let sortedByBirthYear = inventors.sort((first, year) => first.year - year.year);
...
let firstLast = people.map((e) =>{
    let splitArray = e.split(',');
    let last = splitArray[0];
    let first = splitArray[1]
    return `${first} ${last}`;
})
